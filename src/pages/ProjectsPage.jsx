            ...
            return (
              <div key={project.id} className="project-card-anim opacity-100">
                <TiltCard className="brutalist-panel rounded-3xl overflow-hidden border border-white/10 hover:border-white/40 flex flex-col justify-between h-full group transition-all duration-300">
                  ...
                  {/* Action Buttons (2 Buttons: Live Preview iframe modal & Code Repo) */}
                  <div className="p-6 sm:p-7 pt-0 flex flex-wrap items-center gap-3">
                    {siteKey ? (
                      <button
                        onClick={() => openLiveWebsite(siteKey)}
                        className="flex-1 min-w-[130px] inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl bg-white text-black font-mono font-bold text-[12px] hover:bg-zinc-200 transition-all shadow-md hover:scale-105 cursor-pointer"
                        title={`Preview ${project.name} in interactive live viewer`}>
